...
def write_BIPChannelGroups_ALLpsd(incl_sub: list, signalFilter: str, normalization: str, freqBand: str):
    """
    ...
    BIPChannelGroups_ALLpsd_filepath = os.path.join(
        results_path, f"BIPChannelGroups_ALL_{freqBand}_{normalization}_{signalFilter}.pickle"
    )
    with open(BIPChannelGroups_ALLpsd_filepath, "wb") as file:
        pickle.dump(psdAverage_dataframe, file)

    return {"psdAverage_dataframe": psdAverage_dataframe}


def write_BIPChannelGroups_psdRanks_relToRank1(signalFilter: str, normalization: str, freqBand: str):
    """
    ...
    loaded_Dataframe = loadResults.load_BIPChannelGroups_ALL(
        freqBand=freqBand, normalization=normalization, signalFilter=signalFilter
    )
    ...
    for group in recording_montage_group:
        ...
        for STN in sub_hem_keys:
            ...
            for ses in sessions_per_STN:
                ...
                if group == "circular":
                    dataframe_to_rank = STN_session_dataframe[
                        STN_session_dataframe["recording_montage_group"] == "circular"
                    ]  # containing 6 circular BIP channels

                    # drop the BIP channels which contacts are not adjacent
                    dataframe_to_rank = dataframe_to_rank[
                        dataframe_to_rank.recording_montage.str.contains("03|13|02") == False
                    ]

                elif group == "segm_intralevel":
                    dataframe_to_rank = STN_session_dataframe[
                        STN_session_dataframe["recording_montage_group"] == "segm_intralevel"
                    ]  # containing 6 circular BIP channels

                elif group == "segm_interlevel":
                    dataframe_to_rank = STN_session_dataframe[
                        STN_session_dataframe["recording_montage_group"] == "segm_interlevel"
                    ]  # containing 6 circular BIP channels
                ...
                copyDF[f"relativePSD_to_{freqBand}_Rank1"] = copyDF.apply(
                    lambda row: row[f"{freqBand}_psd"] / beta_rank_1, axis=1
    ...
    ### save the Dataframes with pickle
    BIPChannelGroups_psdRanks_relToRank1_filepath = os.path.join(
        results_path, f"BIPChannelGroups_psdRanks_relToRank1_{freqBand}_{normalization}_{signalFilter}.pickle"
    )
    ...
    return {"psdRank_and_relative_dataframe": psdRank_and_relative_dataframe}


def write_GroupMonopolar_weightedPsdCoordinateDistance_relToRank1(
    incl_sub: list, signalFilter: str, normalization: str, freqBand: str
):
    """
    ...
    for sub in incl_sub:
        for hem in hemispheres:
            ...
            sub_hem_result = loadResults.load_monoRef_weightedPsdCoordinateDistance_pickle(
                sub=sub, hemisphere=hem, freqBand=freqBand, normalization=normalization, filterSignal=signalFilter
            )

            for ses in sessions:
                # check if session exists for a subject
                sub_session_keys = list(
                    sub_hem_result.keys()
                )  # list of fu3m_monopolar_Dataframe, fu12m_bipolar_Dataframe
                ...
                session_DF_copy[f"relativePSD_to_{freqBand}_Rank1"] = session_DF_copy.apply(
    ...
    ### save the Dataframes with pickle
    relToRank1_dataframe_filepath = os.path.join(
        results_path,
    ...
    for sub in incl_sub:
        for hem in hemispheres:
            ...
            for ses in sessions:
                ...
                # if rank = 1.0 define averagedPSD value of the same row as beta_rank_1
                beta_rank_1 = session_DF_copy[session_DF_copy["rank"] == 1.0]  # taking the row containing 1.0 in rank
                beta_rank_1 = beta_rank_1[f"estimated_monopolar_psd_{freqBand}"].values[
                    0
                ]  # just taking psdAverage of rank 1.0

                session_DF_copy[f"relativePSD_to_{freqBand}_Rank1"] = session_DF_copy.apply(
                    lambda row: row[f"estimated_monopolar_psd_{freqBand}"] / beta_rank_1, axis=1
                )  # in each row add to new value psd/beta_rank1
...
def write_monopol_rel_psd_from0To8(incl_sub: list, signalFilter: str, normalization: str, freqBand: str):
    """
    Read from existing pickle file "sub{sub}_{hemisphere}_monoRef_weightedPsdByCoordinateDistance_{freqBand}_{normalization}_{filterSignal}.pickle"
    coumns:
        - index (=Contact)
    ...
    for sub in incl_sub:
        for hem in hemispheres:
            ...
            for ses in sessions:
                ...
                beta_rank_8 = beta_rank_8[f"averaged_monopolar_PSD_{freqBand}"].values[
                    0
                ]  # just taking psdAverage of rank 8.0

                # value of rank 1 after subtracting the value of rank 8
                beta_rank_1 = session_DF_copy[
                    session_DF_copy["Rank8contacts"] == 1.0
                ]  # taking the row containing 1.0 in Rank8contacts
                beta_rank_1 = beta_rank_1[f"averaged_monopolar_PSD_{freqBand}"].values[
                    0
                ]  # just taking psdAverage of rank 1.0
                beta_rank_1 = (
                ...
                session_DF_copy[f"relativePSD_{freqBand}_from_0_to_1"] = session_DF_copy.apply(
                    lambda row: (row[f"averaged_monopolar_PSD_{freqBand}"] - beta_rank_8) / beta_rank_1, axis=1
                )
                ...
                # in each row add in new column: (psd-beta_rank_8)/beta_rank1
                session_DF_copy[f"relativePSD_{freqBand}_to_mean_std"] = session_DF_copy.apply(
                    lambda row: (row[f"averaged_monopolar_PSD_{freqBand}"] - mean) / std, axis=1
                )
    ...
    for idx, row in BestClinicalContacts.iterrows():
        ...
        sub_hem_ses_rows = relToRank1_dataframe.loc[
            (relToRank1_dataframe["subject_hemisphere"] == sub_hem)
            & (relToRank1_dataframe["session"] == session)
            & (relToRank1_dataframe["contact"].isin(activeContacts_list))
        ]
    ...
    for idx, row in BestClinicalContacts.iterrows():
        ...
        sub_hem_ses_rows = relToRank1_dataframe.loc[
            (relToRank1_dataframe["subject_hemisphere"] == sub_hem)
            & (relToRank1_dataframe["session"] == session)
            & (relToRank1_dataframe["contact"].isin(inactiveContacts_list))
        ]
        # add a column and fill the cell with the current Polarity
        sub_hem_ses_rows_copy = sub_hem_ses_rows.copy()
    ...
    ### save the Dataframes with pickle
    active_and_inactive_MonoBeta8Ranks_filepath = os.path.join(
        results_path, f"monopol_rel_psd_from0To8_{freqBand}_{normalization}_{signalFilter}.pickle"
    )
...
def write_SSD_filtered_groupChannels(incl_sub=list, f_band=str):
    """
            ...
            for ses in incl_session:
                for cond in incl_cond:
                    for recording_group in incl_contact[f"{hem}"]:
                        ...
                        timedomain_data = getattr(
                            timedomain_data.rest, recording_group
                        )  # for each rec group in incl_contact
                        ...
                        for c, chan in enumerate(ch_names):
                            SSD_results_storage[f"{sub}_{hem}_{ses}_{chan}"] = [
                                sub,
                                hem,
                                ses,
                                recording_group,
    ...
    print("file: ", f"SSD_results_Dataframe_{f_band}.pickle", "\nwritten in: ", results_path)

    return SSD_results_Dataframe


def write_fooof_group_json(incl_sub: list, fooof_version: str):
    """
    ...
    for sub in incl_sub:
        ...
        # create filename
        filename = f"fooof_model_sub{sub}_{fooof_version}.json"
    ...
    group_fooof_dataframe.to_json(os.path.join(results_path_group, f"fooof_model_group_data_{fooof_version}.json"))

    return group_fooof_dataframe


def highest_beta_channels_fooof(fooof_spectrum: str, fooof_version: str):
    """
...
def write_ses_comparison_power_spectra(incl_sub: list, incl_channels: str, signalFilter: str, normalization: str):
    """
    ...
    if incl_channels == "SegmInter":
        ...
        channels = [
            ...
            '12',
            '23',
        ]  # taking the grand average of these channels probably not a good idea bc different impedances of contacts
        ...
        for hem in hemispheres:
            ...
            for ses in sessions:
                ...
                for chan in channels:
                    ...
                    single_channels_dict[f"{sub}_{hem}_{ses}_{chan}"] = [
                        sub,
                        hem,
                        ses,
                        chan,
    ...
    compare_sessions = ["postop_fu3m", "postop_fu12m", "postop_fu18m", "fu3m_fu12m", "fu3m_fu18m", "fu12m_fu18m"]
    ...
    comparisons_storage_filepath = os.path.join(
        results_path, f"power_spectra_{signalFilter}_{incl_channels}_{normalization}_session_comparisons.pickle"
    )
