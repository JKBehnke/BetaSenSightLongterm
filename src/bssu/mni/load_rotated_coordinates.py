...
def load_mni_coordinates(incl_sub: list):
    ...
    for sub in incl_sub:
        ...
        data_path = os.path.join(
            current_path, 'ResearchProjects', 'BetaSenSightLongterm', 'data', 'imagingData', f"sub-{sub}"
        )
        ...
        for h, hem in enumerate(hemispheres):
            ...
            reco_native_DF.insert(
                0, "subject_hemisphere", f"{sub}_{hem}"
            )  # insert column with subject_hemisphere on first position
    ...
    Excel_path = os.path.join(
        ...
        'data',
        'imagingData',
        'SenSightElectrode_coordinates.xlsx',
    )
...
def calculate_mean_coordinates_bipolarRecordings(incl_sub: list, coordinates: str):
    ...
    for sub in incl_sub:
        ...
        for hem in hemispheres:
            ...
            psdAverage_dataframe["subject_hemisphere_recording"] = psdAverage_dataframe[
                ["subject_hemisphere", "recording_montage"]
            ].agg('_'.join, axis=1)
    ...
    for sub in incl_sub:
        ...
        for h, hem in enumerate(hemispheres):
            ...
            reco_DF = pd.DataFrame(
                reco_hem, columns=[f"reco_{coordinates}_x", f"reco_{coordinates}_y", f"reco_{coordinates}_z"]
            )
            reco_DF.insert(
                0, "subject_hemisphere", f"{sub}_{hem}"
            )  # insert column with subject_hemisphere on first position
    ...
    repeat = len(
        sub_hem_unique
    )  # add contacts 0, 1A, 1B etc to dataframe repeatedly depending on how many sub_hem in dataframe
    ...
    for sub_hem in sub_hem_unique:
        for recording in recording_montage_list:
            ...
            x_mean_coordinate[f"{sub_hem}_{recording}"] = np.mean(
                [x_val[f"{sub_hem}_{rec[0]}"], x_val[f"{sub_hem}_{rec[1]}"]]
            )
            y_mean_coordinate[f"{sub_hem}_{recording}"] = np.mean(
                [y_val[f"{sub_hem}_{rec[0]}"], y_val[f"{sub_hem}_{rec[1]}"]]
            )
            z_mean_coordinate[f"{sub_hem}_{recording}"] = np.mean(
                [z_val[f"{sub_hem}_{rec[0]}"], z_val[f"{sub_hem}_{rec[1]}"]]
            )
    ...
    montageType_specific = [
        "circular_ring_ring",
        "circular_ring_segm_long",
        "circular_ring_segm_long",
        "circular_segm_segm",
        "circular_ring_segm_short",
        "circular_ring_segm_short",
        "segm_intralevel",
        "segm_intralevel",
        "segm_intralevel",
    ...
    merged_Dataframe = mean_xyz_coord.merge(
        psdAverage_dataframe, left_on="subject_hemisphere_recording", right_on="subject_hemisphere_recording"
    )
    ...
    Excel_path = os.path.join(
        ...
        'data',
        'imagingData',
        f'SenSightElectrode_MEANcoordinates_{coordinates}.xlsx',
    )
...
def mean_coordinates_beta_bipolar(coordinates: str):
    ...
    fooof_group_result = loadResults.load_fooof_beta_ranks(
        fooof_spectrum="periodic_spectrum", all_or_one_chan="beta_ranks_all"
    )
    ...
    fooof_group_copy = fooof_group_copy.drop(
        columns=[
            "fooof_error",
            "fooof_r_sq",
            "fooof_exponent",
    ...
    fooof_group_copy["subject_hemisphere_recording"] = fooof_group_copy[["subject_hemisphere", "bipolar_channel"]].agg(
        '_'.join, axis=1
    )
    ...
    for sub in incl_sub:
        ...
        if os.path.exists(
            os.path.join(current_path, 'ResearchProjects', 'BetaSenSightLongterm', 'data', 'imagingData', f"sub-{sub}")
        ):
    ...
    for sub_hem in sub_hem_unique:  # looping through all existing sub hem that have mni coordinates
        ...
        for ses in ses_unique:
            ...
            ses_table = sub_hem_beta_table[
                sub_hem_beta_table.session == ses
            ]  # only one subject_hemisphere and unique session
            merged_sub_hem_ses = sub_hem_mni_table.merge(
                ses_table, left_on="subject_hemisphere_recording", right_on="subject_hemisphere_recording"
            )
