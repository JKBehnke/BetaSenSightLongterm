...
def weight_power_by_distance(stn_data: pd.DataFrame, distance_dict: dict, spectra_column: str):
    ...
    return sum_lfp_power / sum_distances  # weighted power of one level or one directional contact
...
def detec_rank_level_and_direction(fooof_version: str):
    ...
    for ses in INCL_SESSIONS:
        ...
        session_Dataframe = weighted_group_results.loc[weighted_group_results.session == ses]
    ...
    helpers.save_result_dataframe_as_pickle(
        data=beta_all_directional_ranks, filename=f"fooof_detec_beta_all_directional_ranks_{fooof_version}"
    )
