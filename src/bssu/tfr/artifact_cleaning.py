...
def load_mne_object_pyPerceive(sub: str, session: str, channel_group: str):
    ...
    temp_data = getattr(
        mainclass_sub.survey, session
    )  # gets attribute e.g. of tp "postop" from modality_class with modality set to survey
    temp_data = getattr(temp_data, "m0s0")  # gets attribute e.g. "m0s0"
    temp_data = getattr(temp_data.rest, channel_group)
    temp_data = (
        temp_data.run1.data
    )  # gets the mne loaded data from the perceive .mat BSSu, m0s0 file with task "RestBSSuRingR"
...
def plot_ieeg_data(sub: str, session: str, channel_group: str, ieeg_data, fig_title: str):
    ...
    # save figure
    fig.savefig(
        os.path.join(figures_path, f"{fig_title}_sub-{sub}_{hem}_{session}_m0s0_{group_name}.svg"),
        bbox_inches="tight",
        format="svg",
    )
    fig.savefig(
        os.path.join(figures_path, f"{fig_title}_sub-{sub}_{hem}_{session}_m0s0_{group_name}.png"), bbox_inches="tight"
    )

    return {"data": ieeg_data, "channels": channels, "hemisphere": hem}
...
def plot_raw_time_series_before_MNE(sub: str, session: str, channel_group: str):
    ...
    # save figure
    fig.savefig(
        os.path.join(figures_path, f"raw_time_series_sub-{sub}_{hem}_{session}_m0s0_{group_name}.svg"),
        bbox_inches="tight",
        format="svg",
    )
    fig.savefig(
        os.path.join(figures_path, f"raw_time_series_sub-{sub}_{hem}_{session}_m0s0_{group_name}.png"),
        bbox_inches="tight",
    )

    return {"data": temp_data.get_data(), "channels": channels, "hemisphere": hem}
...
def plot_ica_components(data):
    ...
    return {"fig_ecg_components": fig, "ica_components": ica_components, "n_components": n_components, "ica": ica}
...
def get_input_ecg_component(message: str) -> str:
    ...
    while True:
        ...
        if int(user_input) in {0, 1, 2, 3, 4, 5, 6}:
            break
        print(f"Input must be an integer from 0 to 6. Got: {user_input}." " Please provide a valid input.")
    ...
    for ses in sessions:
        ...
        for group in CHANNEL_GROUPS:
            ...
            if input_y_or_n_artifact == "n":
                ...
                clean_time_series_dict[f"{sub}_{hem}_{ses}_{group}"] = [sub, hem, ses, group, "no", data]
            ...
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(figures_path, f"ica_components_sub-{sub}_{ses}_{group}.svg"),
                bbox_inches="tight",
                format="svg",
            )
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(figures_path, f"ica_components_sub-{sub}_{ses}_{group}.png"), bbox_inches="tight"
            )
            ...
            if input_y_or_n_clean == "n":
                ...
                if second_input_y_or_n == "y":
                    ...
                    second_plot_ica["fig_ecg_components"].savefig(
                        os.path.join(figures_path, f"second_run_ica_components_sub-{sub}_{ses}_{group}.png"),
                        bbox_inches="tight",
                    )
                    ...
                    if second_input_ecg_component in {1, 2, 3, 4, 5, 6}:
                        ...
                        cleaned_ieeg_data = remove_ecg_artifact_from_signals(
                            cleaned_ieeg_data, second_ecg_component, second_ica_components, second_ica
                        )

                        # Step 4: plot the cleaned signals ############################################
                        plot_cleaned = plot_ieeg_data(
                            sub, ses, group, cleaned_ieeg_data, fig_title="second_run_cleaned_time_series"
                        )
            ...
            clean_time_series_dict[f"{sub}_{hem}_{ses}_{group}"] = [sub, hem, ses, group, "yes", cleaned_ieeg_data]
...
def plot_raw_time_series(incl_sub: list, incl_session: list, incl_condition: list):
                ...
                for g, group in enumerate(channel_groups):
                    ...
                    for c, cond in enumerate(incl_condition):
                        ...
                        for i, ch in enumerate(ch_names):
                            ...
                            artifact_dict[f"{sub}_{hem}_{tp}_{group_name}_{cond}_{channels[i]}"] = [
                                sub,
                                hem,
                                tp,
                                group_name,
                        ...
                        # save figure
                        fig.savefig(
                            os.path.join(figures_path, f"raw_time_series_sub-{sub}_{hem}_{tp}_{cond}_{group_name}.png"),
                            bbox_inches="tight",
                        )
    ...
    move_artifact_result_df.rename(
        index={
            0: "subject",
            1: "hemisphere",
            2: "session",
    ...
    move_artifact_result_df["subject_hemisphere"] = (
        move_artifact_result_df["subject"] + "_" + move_artifact_result_df["hemisphere"]
    )
