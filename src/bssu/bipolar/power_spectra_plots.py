...
def PowerSpectra_perChannel(
    ...
    for norm in normalization:
        ...
        fig.savefig(
            os.path.join(
                subject_figures_path, f"PowerSpectraPerChannel_sub{sub}_{hemisphere}_{norm}_{signalFilter}.png"
            ),
            bbox_inches="tight",
        )  # bbox_inches makes sure that the title won´t be cut off


def PowerSpectra_perChannelGroup(
    sub: str,
    hemisphere: str,
    incl_session: list,
    signalFilter: str,
    ...
    for norm in normalization:
        ...
        fig.savefig(
            os.path.join(
                subject_figures_path, f"PowerSpectraPerChannelGroup_sub{sub}_{hemisphere}_{norm}_{signalFilter}.png"
            ),
            bbox_inches="tight",
        )  # bbox_inches makes sure that the title won´t be cut off


def power_spectra_grand_average_per_session(incl_sub: list, channel_group: str, signalFilter: str, normalization: str):
    """
        ...
        for hem in hemispheres:
            ...
            stn_power_spectra = mainAnalysis_class.MainClass(
                ...
                feature=[
                    ...
                    "SEM_rawPsd",
                    "normPsdToTotalSum",
                    "SEM_normPsdToTotalSum",
                    "normPsdToSumPsd1to100Hz",
                    "SEM_normPsdToSumPsd1to100Hz",
                    "normPsdToSum40to90Hz",
                    "SEM_normPsdToSum40to90Hz",
                ],
            )
    ...
    for ses in sessions:
        ...
        # save and return
        average_spectra[f"{ses}"] = [
            ses,
            frequencies,
            power_spectrum_session_grand_average,
            standard_deviation_session,
            sem_session,
    ...
    fig.savefig(
        os.path.join(figures_path, f"grand_average_power_spectra_{channel_group}_{normalization}_{signalFilter}.png"),
        bbox_inches="tight",
    )
...
def grand_average_FOOOF_spectra(std_or_sem: str, spectrum_to_plot: str, highest_beta_chans_only: str):
    """
    ...
    for group in channel_group:
        ...
        for ses in sessions:
            ...
            power_spectrum_session_grand_average = df_transposed.apply(
                lambda x: np.mean(x), axis=0
            )  # mean along columns: for each position (1-95) -> outcome=pd.Series with 95 values
        ...
        if highest_beta_chans_only == "yes":
            plt.title(
                f"Grand average FOOOF power spectra of \nonly highest beta channels of the {group} group",
                fontdict={"size": 18},
            )
        ...
        fig.savefig(
            os.path.join(figures_path, f"grand_average_FOOOF_{all_or_one}_{group}_{spectrum_to_plot}_{std_or_sem}.png"),
            bbox_inches="tight",
        )
...
def FOOOF_spectra_per_channel(incl_sub: str, fooof_version: str):
    """
    Plot an average power spectrum of FOOOF modeled spectra per session for each channel group.
    ...
    for stn in stn_list:
        ...
        cycler_colors = cycler(
            "color", ["turquoise", "sandybrown", "plum", "cornflowerblue", "tab:grey", "yellowgreen"]
        )
        plt.rc('axes', prop_cycle=cycler_colors)
        ...
        for group in channel_group:
            ...
            for c, chan in enumerate(eval(group)):
                ...
                for ses in sessions:
                    ...
                    if fooof_version == "v2":
                        frequencies = np.arange(2, 96)  # 2-95 Hz, 1 Hz resolution

                    elif fooof_version == "v1":
                        frequencies = np.arange(1, 96)  # 1-95 Hz, 1 Hz resolution
            ...
            for s, ses in enumerate(sessions):  # 0,1,2,3,5
                ...
                for c, chan in enumerate(eval(group)):
                    ...
                    if fooof_version == "v2":
                        ...
                        frequencies = np.arange(1, 96)  # 2-95 Hz, 1 Hz resolution
