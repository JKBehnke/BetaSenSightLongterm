...
def MonoRef_JLB(incl_sub: str, hemisphere: str, normalization: str, incl_session: list):
    ...
    for t, tp in enumerate(time_points):
        ...
        for f, fq in enumerate(frequency_range):
            ...
            session_frequency_Dataframe = psdAverageDataframe[
                (psdAverageDataframe["frequencyBand"] == fq) & (psdAverageDataframe["session"] == tp)
            ]
    ...
    psdAverageDF.rename(
        index={0: "session", 1: "frequency_band", 2: "channel", 3: "averagedPSD"}, inplace=True
    )  # rename the rows
    ...
    psdPercentageDF.rename(
        index={0: "session", 1: "frequency_band", 2: "direction", 3: "percentagePSD_perDirection"}, inplace=True
    )  # rename the rows
    ...
    MonoRef_JLB_result_filepath = os.path.join(
        results_path, f"sub{incl_sub}_{hemisphere}_MonoRef_JLB_result_{normalization}_band-pass.pickle"
    )
...
def MonoRefPsd_highestRank(sub: str, normalization: str, hemisphere: str):
    ...
    # read .csv file as Dataframe
    monoRef_result = loadcsv.load_MonoRef_JLBresultCSV(sub=sub, normalization=normalization, hemisphere=hemisphere)
    ...
    monopolarFirstRankDF = pd.DataFrame(
        list(monopolarFirstRank.items()), columns=['session_frequencyBand', 'numberOneRank_monopolarChannel']
    )
    ...
    FirstRankChannel_PSD_DF.to_csv(
        os.path.join(results_path, f"FirstRankChannel_PSD_DF{normalization}_{hemisphere}"), sep=","
    )
    postopBaseline_beta.to_csv(os.path.join(results_path, f"postopBaseline_beta{normalization}_{hemisphere}"), sep=",")
    postopBaseline_lowBeta.to_csv(
        os.path.join(results_path, f"postopBaseline_lowBeta{normalization}_{hemisphere}"), sep=","
    )
    postopBaseline_highBeta.to_csv(
        os.path.join(results_path, f"postopBaseline_highBeta{normalization}_{hemisphere}"), sep=","
    )
    fu3mBaseline_beta.to_csv(os.path.join(results_path, f"fu3mBaseline_beta{normalization}_{hemisphere}"), sep=",")
    fu3mBaseline_lowBeta.to_csv(
        os.path.join(results_path, f"fu3mBaseline_lowBeta{normalization}_{hemisphere}"), sep=","
    )
    fu3mBaseline_highBeta.to_csv(
        os.path.join(results_path, f"fu3mBaseline_highBeta{normalization}_{hemisphere}"), sep=","
    )
...
def fooof_monoRef_JLB(fooof_version: str):
    ...
    MonoRef_JLB_result_filepath = os.path.join(results_path, f"MonoRef_JLB_fooof_beta_{fooof_version}.pickle")
    with open(MonoRef_JLB_result_filepath, "wb") as file:
        pickle.dump(monopolar_results_all, file)

    print(f"MonoRef_JLB_fooof_beta_{fooof_version}.pickle", f"\nwritten in: {results_path}")
