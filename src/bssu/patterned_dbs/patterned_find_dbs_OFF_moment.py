...
def find_LFP_sync_artefact(
    ...
    kernels = {'1': np.array([1, -1]), '2': np.array([1, 0, -1] + list(np.linspace(-1, 0, 20)))}
    ...
    if consider_first_seconds_LFP:
        ...
        sel = np.logical_or(np.array(stim_idx) < border_start, np.array(stim_idx) > border_end)
    ...
    if not signal_inverted:
        sel_idx = np.array([min(lfp_data[i - 5 : i + 5]) for i in stim_idx]) < (np.median(abs_heights) * -0.5)
    elif signal_inverted:
        sel_idx = np.array([max(lfp_data[i - 5 : i + 5]) for i in stim_idx]) > (np.median(abs_heights) * 0.5)
    stim_idx = list(compress(stim_idx, sel_idx))
