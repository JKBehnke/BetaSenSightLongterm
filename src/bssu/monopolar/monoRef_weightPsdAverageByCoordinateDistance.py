...
def monoRef_weightPsdBetaAverageByCoordinateDistance(
    sub: str, hemisphere: str, filterSignal: str, normalization: str, freqBand: str, incl_sessions: list
):
    """
    ...
    originalPsdAverageDataframe = loadResults.load_PSDjson(
        sub=sub, result="PSDaverageFrequencyBands", hemisphere=hemisphere, filter=filterSignal
    )
    ...
    originalPsdAverageDataframe = originalPsdAverageDataframe[
        originalPsdAverageDataframe.absoluteOrRelativePSD == normalization
    ]
    ...
    for ses in incl_sessions:
        ...
        for contact in contact_coordinates.keys():
            ...
            mono_data_psdAverage.loc[contact, f'averaged_monopolar_PSD_{freqBand}'] = np.sum(
                weighted_beta
            )  # sum of all 15 weighted psdAverages = one monopolar contact psdAverage

        # ranking the weighted monopolar psd
        mono_data_psdAverage["rank"] = mono_data_psdAverage[f"averaged_monopolar_PSD_{freqBand}"].rank(
            ascending=False
    ...
    session_data_filepath = os.path.join(
        results_paths,
        f"sub{sub}_{hemisphere}_monoRef_weightedPsdByCoordinateDistance_{freqBand}_{normalization}_{filterSignal}.pickle",
    )
    with open(session_data_filepath, "wb") as file:
        pickle.dump(session_data, file)

    print(
        f"New file: sub{sub}_{hemisphere}_monoRef_weightedPsdByCoordinateDistance_{freqBand}_{normalization}_{filterSignal}.pickle",
...
def monoRef_only_segmental_weight_psd_by_distance(
    ...
    for ses in incl_sessions:
        ...
        for contact in segmental_contacts:
            ...
            # weighting the beta of bipolar contacts by their similarity to the monopolar contact
            weighted_beta = (
                session_Dataframe_coord['averagedPSD'].values * similarity
            )  # two arrays with same length = 9 bip_chans

            # storing the weighted beta for the mono polar contact
            mono_data_psdAverage.loc[contact, f'estimated_monopolar_psd_{freqBand}'] = np.sum(
                weighted_beta
        ...
        # ranking the weighted monopolar psd
        mono_data_psdAverage["rank"] = mono_data_psdAverage[f"estimated_monopolar_psd_{freqBand}"].rank(
            ascending=False
        )  # rank highest psdAverage as 1.0
    ...
    session_data_filepath = os.path.join(
        results_paths,
        f"sub{sub}_{hemisphere}_monoRef_only_segmental_weight_psd_by_distance{freqBand}_{normalization}_{filterSignal}.pickle",
    )
    with open(session_data_filepath, "wb") as file:
        pickle.dump(session_data, file)

    print(
        f"New file: sub{sub}_{hemisphere}_monoRef_only_segmental_weight_psd_by_distance{freqBand}_{normalization}_{filterSignal}.pickle",
        f"\nwritten in: {results_paths}",
    )
...
#                 # weighting the beta of bipolar contacts by their similarity to the monopolar contact
#                 weighted_beta = (
#                     stn_ses_bipolar['beta_average'].values * similarity
#                 )  # two arrays with same length = 9 bip_chans
...
#     session_data_filepath = os.path.join(
#         results_paths,
#         f"fooof_monoRef_{filename}weight_beta_psd_by_{similarity_calculation}_{fooof_spectrum}_{fooof_version}.pickle",
#     )
#     with open(session_data_filepath, "wb") as file:
#         pickle.dump(session_data, file)

#     print(
#         f"New file: fooof_monoRef_{filename}weight_beta_psd_by_{similarity_calculation}_{fooof_spectrum}_{fooof_version}.pickle",
#         f"\nwritten in: {results_paths}",
#     )
...
#     # save session_data dictionary with bipolar and monopolar psd average Dataframes as pickle files
#     all_ranked_datapath = os.path.join(
#         results_paths, f"fooof_monoRef_all_contacts_weight_beta_psd_by_{similarity_calculation}_{fooof_version}.pickle"
#     )
#     with open(all_ranked_datapath, "wb") as file:
#         pickle.dump(all_ranked_data, file)

#     print(
#         f"New file: fooof_monoRef_all_contacts_weight_beta_psd_by_{similarity_calculation}_{fooof_version}.pickle",
#         f"\nwritten in: {results_paths}",
#     )
...
def calculate_average_coordinates_between_contacts(session_Dataframe_coord, only_segmental, contact_coordinates):
    ##################### looping over bipolar channels, get average coordinates between 2 contacts #####################
    for idx in session_Dataframe_coord.index:
        # extracting contact names
        bipolar_channel = session_Dataframe_coord.loc[idx, 'bipolar_channel']  # e.g. 1A2A
...
def weight_power_of_single_contacts(mono_data, mono_data_copy, stn_ses_bipolar, contacts, similarity_calculation):
    ...
    for contact in contacts:
        ...
        beta_from_weighted_power = np.mean(sum_weighted_power[11:34])  # 2-95 Hz = 94 values
        # index 0 = 2 Hz, index 11 = 13 Hz, index 33 = 35 Hz

        # storing the weighted power for the mono polar contact
        mono_data_copy.loc[contact, 'estimated_monopolar_beta_psd'] = beta_from_weighted_power
