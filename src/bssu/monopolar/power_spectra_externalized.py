...
def get_bids_id_from_sub_hem(subject_hemisphere: str):
    ...
    bids_id = PATIENT_METADATA.loc[PATIENT_METADATA["patient_ID"] == int(sub_wo_zero), "BIDS_key"].values[0]
...
def plot_power_spectra_monopolar(method: str, fooof: str, only_directional: str):
    ...
    # load data
    loaded_data = loadResults.load_pickle_group_result(filename=filename_load_data, fooof_version="v2")
    ...
    for sub_hem in subject_hemisphere_unique:
        ...
        helpers.save_fig_png_and_svg(
            path=figures_path, filename=f"power_spectra_{method}_{sub_hem}_{directionality_filename}", figure=fig
        )
