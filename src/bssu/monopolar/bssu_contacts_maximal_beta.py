...
directional_recordings = ["1A1B", "1B1C", "1A1C", "2A2B", "2B2C", "2A2C", "1A2A", "1B2B", "1C2C"]
...
def select_directional_contact_pair_from_bssu(fooof_version: str):
    ...
    bssu_beta_data = bssu_beta_data.loc[bssu_beta_data["bipolar_channel"].isin(directional_recordings)]
    ...
    for ses in incl_sessions:
        ...
        for sub_hem in stn_unique_ses:
            ...
            sub_hem_data_copy["beta_rank"] = sub_hem_data_copy["beta_average"].rank(ascending=False)

            # save in a dataframe
            ranked_directional_bssu_recordings = pd.concat([ranked_directional_bssu_recordings, sub_hem_data_copy])

            # now only select the bipolar recording with rank 1
            rank_1_directional_recording = sub_hem_data_copy.loc[sub_hem_data_copy.beta_rank == 1.0]
            rank_1_directional_recording = rank_1_directional_recording.bipolar_channel.values[0]
    ...
    results_dataframe.rename(index={0: "session", 1: "subject_hemisphere", 2: "selected_2_contacts"}, inplace=True)
    results_dataframe = results_dataframe.transpose()

    # save monopolar psd estimate Dataframes as pickle files
    results_filepath = os.path.join(results_path, f"best_2_contacts_from_directional_bssu_{fooof_version}.pickle")
    with open(results_filepath, "wb") as file:
        pickle.dump(results_dataframe, file)

    print(f"best_2_contacts_from_directional_bssu_{fooof_version}.pickle", f"\nwritten in: {results_path}")
...
def select_directional_contact_pair_from_externalized_bssu(fooof_version: str):
    ...
    # Load the bipolar beta values (FOOOF)
    bssu_beta_data = load_data.load_externalized_pickle(
    ...
    for sub_hem in stn_unique:
        ...
        sub_hem_data_copy["beta_average"] = sub_hem_data_copy["beta_average"].apply(lambda row: np.mean(row[13:36]))
        sub_hem_data_copy["beta_rank"] = sub_hem_data_copy["beta_average"].rank(ascending=False)
    ...
    results_filepath = os.path.join(
        results_path, f"best_2_contacts_from_directional_externalized_bssu_{fooof_version}.pickle"
    )
    with open(results_filepath, "wb") as file:
        pickle.dump(results_dataframe, file)

    print(f"best_2_contacts_from_directional_externalized_bssu_{fooof_version}.pickle", f"\nwritten in: {results_path}")
