                ...
                for cond in incl_cond:
                    for recording_group in incl_contact[f"{hem}"]:
                        ...
                        for c, chan in enumerate(ch_names):
                            ...
                            axes[0].set_title(
                                f'Power Spectra {recording_group}: sub{sub} {hem} hemisphere, {ses}', fontsize=20
                            )
                        ...
                        f, psd = signal.welch(
                            ssd_filt_data, axis=-1, nperseg=sfreq, fs=sfreq
                        ...
                        fig.savefig(
                            sub_results_path
                            + f"\\sub-{sub}_{hem}_SSD_{f_band}_firstComponent_PowerSpectrum_{ses}_{recording_group}"
                        )
