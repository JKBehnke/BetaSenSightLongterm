...
def group_monopolarPSDaverageAndRanks(
    ...
    for sub, subject in enumerate(incl_sub):
        for hem, hemisphere in enumerate(incl_hemisphere):
            ...
            monopolPsd.rename(
                columns={"Unnamed: 0": "Monopolar_contact"}, inplace=True
            )  # rename the column to Monopolar contact
            ...
            for ind, freq in enumerate(frequencyBand):
                ...
                filteredPSD_DF.insert(
                    0, "subject_hemisphere", f"sub{subject}_{hemisphere}", True
                )  # insert new column as first column with subject and hemisphere details
    ...
    monopolPSD_highBeta_ALL.to_csv(
        os.path.join(results_path_general, f"monopolPSD_highBeta_ALL{normalization}"), sep=","
    )
    monopolRanks_beta_ALL.to_csv(os.path.join(results_path_general, f"monopolRanks_beta_ALL{normalization}"), sep=",")
    monopolRanks_lowBeta_ALL.to_csv(
        os.path.join(results_path_general, f"monopolRanks_lowBeta_ALL{normalization}"), sep=","
    )
    monopolRanks_highBeta_ALL.to_csv(
        os.path.join(results_path_general, f"monopolRanks_highBeta_ALL{normalization}"), sep=","
    )
...
def monopol_psd_correlations_sessions(freqBand: str, ranks_or_relPsd: str, mean_or_median: str):
    """
    ...
    data_weightedByCoordinates = loadResults.load_GroupMonoRef_weightedPsdCoordinateDistance_pickle(
        freqBand=freqBand, normalization="rawPsd", filterSignal="band-pass"
    )
    ...
    for STN in sub_hem_keys:
        ...
        for ses in sessions:
            ...
            STN_session_data["Rank8contacts"] = STN_session_data["averaged_monopolar_PSD_beta"].rank(
                ascending=False
            )  # ranks 1-8
        ...
        for STN in sub_hem_keys:
            ...
            if ranks_or_relPsd == "ranks":
                ...
                spearman_correlation = stats.spearmanr(
                    STN_session_1.Rank8contacts.values, STN_session_2.Rank8contacts.values
                )
                ...
                spearman_correlation = stats.spearmanr(
                    STN_session_1.relativePSD_to_beta_Rank1from8.values,
                    STN_session_2.relativePSD_to_beta_Rank1from8.values,
                )

            # dictionary to store the spearman r values and pval per STN and session combination
            session_pair_STNs[f"{session_1}_{session_2}_{STN}"] = [
                session_1,
                session_2,
                f"{session_1}_{session_2}",
                STN,
                spearman_correlation.statistic,
                spearman_correlation.pvalue,
            ]
    ...
    spearman_m_df.rename(
        index={0: "session_1", 1: "session_2", 2: f"{mean_or_median}_spearmanr", 3: f"{mean_or_median}_pval"},
        inplace=True,
    )
    ...
    for i in range(spearmanr_to_plot.shape[0]):
        for j in range(spearmanr_to_plot.shape[1]):
            plt.text(
                j + 0.5, i + 0.5, str("{: .2f}".format(spearmanr_to_plot[i, j])), ha='center', va='center'
    ...
    fig.tight_layout()
    fig.savefig(
        figures_path + f"\\monopol_session_correlations_heatmap_{freqBand}_{ranks_or_relPsd}_{mean_or_median}.png",
        bbox_inches="tight",
    )

    return {"results_DF": results_DF, "spearman_m_df": spearman_m_df, "sample_size": sample_size_df}


def fooof_monopol_psd_spearman_betw_sessions(
    mean_or_median: str, only_segmental: str, values_to_correlate: str, similarity_calculation: str, fooof_version: str
):
    """
    Load file:
    containing DF with all monopolar PSD estimates in a frequency band, their ranks along an electrode and their PSD relative to the highest PSD of an electrode.
    ...
    results_DF_copy.to_excel(
        os.path.join(
            results_path, f"fooof_monopol_{similarity_calculation}_beta_correlations_per_stn_{fooof_version}.xlsx"
        ),
    ...
    print(
        "file: ",
        f"fooof_monopol_{similarity_calculation}_beta_correlations_per_stn_{fooof_version}.xlsx",
        "\nwritten in: ",
        results_path,
    )
    ...
    spearman_m_df.rename(
        index={
            0: "session_1",
            1: "session_2",
            2: f"{mean_or_median}_spearmanr",
            3: f"{mean_or_median}_pval",
            4: "spearman_std",
        },
        inplace=True,
    )
...
def mono_rank_differences(freq_band: str, normalization: str, filter_signal: str, level_or_direction: str):
    ...
    # load the monolopar beta psd for for each electrode at different timepoints
    data_weightedByCoordinates = loadResults.load_GroupMonoRef_weightedPsdCoordinateDistance_pickle(
        freqBand=freq_band, normalization=normalization, filterSignal=filter_signal
    )
    ...
    for STN in sub_hem_keys:
        ...
        for ses in sessions:
            ...
            # choose only directional contacts and Ring contacts 0, 3 and rank again only the chosen contacts
            STN_session_data = STN_session_data[STN_session_data["contact"].isin(contacts)]
            STN_session_data["rank_contacts"] = STN_session_data["averaged_monopolar_PSD_beta"].rank(
                ascending=False
            )  # ranks 1-8
            STN_session_data_copy = STN_session_data.copy()
            STN_session_data_copy.drop(["rank"], axis=1, inplace=True)
            ...
            # calculate the relative PSD to the highest PSD of the 8 remaining contacts
            beta_rank_1 = STN_session_data_copy[
                STN_session_data_copy["rank_contacts"] == 1.0
            ]  # taking the row containing 1.0 in rank
            beta_rank_1 = beta_rank_1[f"averaged_monopolar_PSD_{freq_band}"].values[
                0
            ]  # just taking psdAverage of rank 1.0

            STN_session_data_copy[f"relativePSD_to_{freq_band}_rank_contacts"] = STN_session_data_copy.apply(
                lambda row: row[f"averaged_monopolar_PSD_{freq_band}"] / beta_rank_1, axis=1
            )  # in each row add to new value psd/beta_rank1
    ...
    weightedByCoordinate_Dataframe = weightedByCoordinate_Dataframe.replace(
    ...
    if level_or_direction == "level":
        weightedByCoordinate_Dataframe_copy = weightedByCoordinate_Dataframe_copy.assign(
            contact_level=weightedByCoordinate_Dataframe_copy["contact"]
        ).rename(columns={"contact_level": "contact_level"})
        weightedByCoordinate_Dataframe_copy["contact_level"] = weightedByCoordinate_Dataframe_copy[
            "contact_level"
        ].replace(
            to_replace=["0", "3"], value=[0, 3]
        )  # level 0 or 3
        weightedByCoordinate_Dataframe_copy["contact_level"] = weightedByCoordinate_Dataframe_copy[
            "contact_level"
        ].replace(
        ...
        weightedByCoordinate_Dataframe_copy = weightedByCoordinate_Dataframe_copy.assign(
            contact_direction=weightedByCoordinate_Dataframe_copy["contact"]
        ).rename(columns={"contact_direction": "contact_direction"})
        weightedByCoordinate_Dataframe_copy["contact_direction"] = weightedByCoordinate_Dataframe_copy[
            "contact_direction"
        ].replace(
            to_replace=["1A", "2A"], value=["A", "A"]
        )  # direction A
        weightedByCoordinate_Dataframe_copy["contact_direction"] = weightedByCoordinate_Dataframe_copy[
            "contact_direction"
        ].replace(
            to_replace=["1B", "2B"], value=["B", "B"]
        )  # direction B
        weightedByCoordinate_Dataframe_copy["contact_direction"] = weightedByCoordinate_Dataframe_copy[
            "contact_direction"
        ].replace(
            to_replace=["1C", "2C"], value=["C", "C"]
        )  # ldirection C
    ...
    for comp in comparisons:
        ...
        for stn in sub_hem_keys:
            ...
            stn_dataframe = weightedByCoordinate_Dataframe_copy.loc[
                weightedByCoordinate_Dataframe_copy.subject_hemisphere == stn
            ]
            ...
            for rank in ranks_range:
                ...
                if level_or_direction == "level":
                    ...
                    level_difference_rank = abs(
                        rank_contact_level_session_1 - rank_contact_level_session_2
                    )  # difference of level as absolute number

                    # store in dictionary
                    difference_dict[f"{comp}_{stn}_{rank}"] = [
                        comp,
                        session_1,
                        session_2,
                        stn,
                        rank,
                        rank_contact_level_session_1,
                        rank_contact_level_session_2,
                        level_difference_rank,
                    ]

                elif level_or_direction == "direction":
                    rank_contact_direction_session_1 = rank_session_1.contact_direction.values[
                        0
                    ]  # direction of rank as str
                    rank_contact_direction_session_2 = rank_session_2.contact_direction.values[
                        0
                    ]  # direction of rank as str
                    ...
                    difference_dict[f"{comp}_{stn}_{rank}"] = [
                        ...
                        stn,
                        rank,
                        rank_contact_direction_session_1,
                        rank_contact_direction_session_2,
                        direction_difference_rank,
                    ]

    # transform dictionary to dataframe
    difference_df = pd.DataFrame(difference_dict)
    difference_df.rename(
        index={
...
def mono_rank_difference_heatmap(
    ...
    for comp in comparisons:
        ...
        # save in a dict
        session_comparison_difference_dict[f"{comp}"] = [
            comp,
            session_1,
            session_2,
            total_rank_comparisons,
            rel_0,
            rel_1,
            rel_2,
            rel_3,
    ...
    session_comparison_difference_df.rename(
        index={
            0: "session_comparison",
            1: "session_1",
            2: "session_2",
    ...
    # Set the x and y ticks to show the indices of the matrix
    ax.set_xticks(
        np.arange(difference.shape[1]) + 0.5, minor=False
    )  # if minor=True it will plot x and y labels differently
    ...
    for i in range(difference.shape[0]):
        for j in range(difference.shape[1]):
            plt.text(
                j + 0.5, i + 0.5, str("{: .2f}".format(difference[i, j])), ha='center', va='center'
            )  # only show 2 numbers after the comma of a float
...
def mono_rank_differences_only_segmental_rec_used(
    ...
    # replace all session names by integers
    data_weightedByCoordinates = data_weightedByCoordinates.replace(
        to_replace=["postop", "fu3m", "fu12m", "fu18m"], value=[0, 3, 12, 18]
    )
...
def fooof_mono_rank_differences(fooof_spectrum: str, level_or_direction_or_rank: str, similarity_calculation: str):
    """
    load the group dataframe of monopolar estimated psd values:
        e.g. group_monoRef_only_segmental_weight_psd_by_distance_beta_rawPsd_band-pass.pickle
    ...
    Input:
        ...
        - level_or_direction_or_rank: str e.g. "level" or "direction" or "rank"
    ...
    if level_or_direction_or_rank == "level":
        ranks_range = [1, 2, 3, 4, 5, 6, 7, 8]
        only_segmental = "no"

    elif level_or_direction_or_rank == "direction":
        ranks_range = [1, 2, 3, 4, 5, 6]
        only_segmental = "yes"
    ...
    if level_or_direction_or_rank == "direction":
        loaded_fooof_monopolar = loadResults.load_fooof_monopolar_weighted_psd(
            fooof_spectrum=fooof_spectrum, segmental=only_segmental, similarity_calculation=similarity_calculation
        )

        fooof_monopolar_df = pd.concat(
            [
                loaded_fooof_monopolar["postop_monopolar_Dataframe"],
    ...
    if level_or_direction_or_rank == "level":
        fooof_monopolar_df_copy = fooof_monopolar_df_copy.assign(
            contact_level=fooof_monopolar_df_copy["contact"]
        ).rename(columns={"contact_level": "contact_level"})
        fooof_monopolar_df_copy["contact_level"] = fooof_monopolar_df_copy["contact_level"].replace(
            to_replace=["0", "3"], value=[0, 3]
        )  # level 0 or 3
        fooof_monopolar_df_copy["contact_level"] = fooof_monopolar_df_copy["contact_level"].replace(
            to_replace=["1A", "1B", "1C"], value=[1, 1, 1]
        ...
        fooof_monopolar_df_copy = fooof_monopolar_df_copy.assign(
            contact_direction=fooof_monopolar_df_copy["contact"]
        ).rename(columns={"contact_direction": "contact_direction"})
        fooof_monopolar_df_copy["contact_direction"] = fooof_monopolar_df_copy["contact_direction"].replace(
            to_replace=["1A", "2A"], value=["A", "A"]
        )  # direction A
        fooof_monopolar_df_copy["contact_direction"] = fooof_monopolar_df_copy["contact_direction"].replace(
            to_replace=["1B", "2B"], value=["B", "B"]
    ...
    for comp in comparisons:
        ...
        for stn in sub_hem_keys:
            ...
            # label if contact rank 1 from session 1 is still rank 1 or 2 at session 2
            if level_or_direction_or_rank == "rank":
                contact_rank1_session1 = stn_session_1.loc[stn_session_1["rank_beta"] == 1]
                contact_rank1_session1 = contact_rank1_session1.contact.values[0]  # contact of rank 1 at session 1

                # look at the same contact at session 2, if rank 1: label as stays_rank_1, if rank 2 label as stays_within_highest_2
                same_contact_session2 = stn_session_2.loc[stn_session_2["contact"] == contact_rank1_session1]
                rank_contact_session2 = same_contact_session2.rank_beta.values[
                    0
                ]  # rank of the same contact in session 2

                if rank_contact_session2 == 1:
                    label_contact_session2 = 0  # stays rank 1 (0 change)

                elif rank_contact_session2 == 2:
                    label_contact_session2 = 1  # rank 2, so change of 1 rank

                else:
                    label_contact_session2 = 2  # rank changed to rank 3 or more

                difference_dict[f"{comp}_{stn}"] = [
                    comp,
                    session_1,
                    session_2,
                    stn,
                    contact_rank1_session1,
                    rank_contact_session2,
                    label_contact_session2,
                ]

            else:
                # go through each rank and calculate the difference of direction or level between two sessions
                for rank in ranks_range:
                    rank_session_1 = stn_session_1.loc[
                        stn_session_1["rank_beta"] == rank
                    ]  # row of one rank of session 1
                    rank_session_2 = stn_session_2.loc[
                        stn_session_2["rank_beta"] == rank
                    ]  # row of one rank of session 2

                    if level_or_direction_or_rank == "level":
                        rank_contact_level_session_1 = rank_session_1.contact_level.values[
                            0
                        ]  # level of rank as integer
                        rank_contact_level_session_2 = rank_session_2.contact_level.values[
                            0
                        ]  # level of rank as integer
    ...
    if level_or_direction_or_rank == "rank":
        ...
        difference_df.rename(
            index={
                ...
                2: "session_2",
                3: "subject_hemisphere",
                4: "contact_rank1_session_1",
                5: "rank_contact_session_2",
                6: "label_contact_session_2",
            },
            inplace=True,
        )
        ...
        difference_df.rename(
            index={
                ...
                3: "subject_hemisphere",
                4: "rank",
                5: f"{level_or_direction_or_rank}_session_1",
                6: f"{level_or_direction_or_rank}_session_2",
                7: f"{level_or_direction_or_rank}_difference",
            },
            inplace=True,
        )
...
def fooof_mono_rank_difference_heatmap(
    ...
    ranks_included: list,
    difference_to_plot: str,
    level_or_direction_or_rank: str,
    similarity_calculation: str,
    label_percentage_or_division: str,
):
    """
    Research question: how many levels do beta ranks change over time across electrodes?
    ...
    Input:
        ...
        - label_percentage_or_division: str "percentage" or "division"
    ...
    if difference_to_plot == "1_or_less":
        ...
        label_with_n = session_comparison_difference_df.rel_string_difference_more_than_0.values
        label_with_n = label_with_n.reshape(4, 4)
        ...
        label_with_n = session_comparison_difference_df.rel_string_difference_1.values
        label_with_n = label_with_n.reshape(4, 4)
        ...
        label_with_n = session_comparison_difference_df.rel_string_difference_2.values
        label_with_n = label_with_n.reshape(4, 4)
        ...
        label_with_n = session_comparison_difference_df.rel_string_difference_3.values
        label_with_n = label_with_n.reshape(4, 4)
    ...
    for i in range(difference.shape[0]):
        for j in range(difference.shape[1]):
            if label_percentage_or_division == "percentage":
                plt.text(
                    j + 0.5, i + 0.5, str("{: .2f}".format(difference[i, j])), ha='center', va='center'
                )  # only show 2 numbers after the comma of a float

            elif label_percentage_or_division == "division":
                plt.text(j + 0.5, i + 0.5, label_with_n[i, j], ha='center', va='center')

    plt.title(
        f"Difference in {level_or_direction_or_rank} of {difference_to_plot} \nof FOOOF beta ranks {ranks_included}"
    )

    fig.tight_layout()

    if level_or_direction_or_rank == "direction":
        fig_name_png = f"fooof_monopol_{similarity_calculation}_only_segm_heatmap_beta_ranks_{ranks_included}_{level_or_direction_or_rank}_difference_{difference_to_plot}_{label_percentage_or_division}.png"
        fig_name_svg = f"fooof_monopol_{similarity_calculation}_only_segm_heatmap_beta_ranks_{ranks_included}_{level_or_direction_or_rank}_difference_{difference_to_plot}_{label_percentage_or_division}.svg"
